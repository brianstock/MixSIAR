...
Here we show how to do a mixing model analysis using the **script** version of MixSIAR. For a demonstration of the **GUI** version, see the [MixSIAR Manual](https://github.com/brianstock/MixSIAR/blob/master/mixsiar_manual_3.0.pdf). For a clean, runnable `.R` script, look at `mixsiar_script_wolves.R` in the `example_scripts` folder of the MixSIAR package install.
...
+ Mixed sediment/water tracer values (sediment/hydrology fingerprinting)
  ...
  `iso_names`, `random_effects`, and `cont_effects` can be a subset of your columns, i.e. have 3 isotopes in file but only want MixSIAR to use 2, or have data by Region and Pack but only want MixSIAR to use Region
...
**Note:** If you have only 1 mix datapoint, you have no information about the mixture/consumer variability. In this case, we ues the original MixSIR error model (which does not fit a residual error term). This is the same behavior as `siarsolo` in SIAR.
