...
vignette: >
  %\VignetteIndexEntry{Ex 1: Wolves}
...
1. Load data files:
    + Mixture (`load_mix_data()`)
    + Sources (`load_source_data()`)
    + Discrimination (TDF) (`load_discr_data()`)
    
2. Pre-model checks
    + Plot your data (`plot_data()`)
    + Calculate convex hull area (`calc_area()`)
    + Plot your prior (`plot_prior()`)

3. Choose model structure options
    + Write JAGS model file (`write_JAGS_model()`)
    
4. Run model
    + Run the JAGS model (`run_model()`)

5. Use diagnostics to decide if the model has converged
    + Check diagnostics (`output_JAGS()`)

6. Analyze output
    + Check summary statistics and posterior density plots (`output_JAGS()`)

## Wolves Example

The "Wolves Example" uses data reconstructed from (not identical to) [Semmens et al. 2009](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0006187). Here, we investigate the diet of 66 wolves in British Columbia with:
...
If 2 isotopes/tracers, calculate normalized surface area of the convex hull polygon(s) as in [Brett (2014)](https://www.int-res.com/articles/suppl/m514p001_supp.pdf).
